...
var PX={
	...
	this.tp;
	this.pgn=0;
	this.browser = function(cb,tp){
	this.callback = cb;this.tp=tp;
	$(".pxmob").html('<img src="'+base_url+'/temp/img/loading.gif" />');this.pgn=0;
	$.ajax({
		url:base_url+"ajax.php",
		data:"class=Media_Core&function=getMediaAjax&tp="+tp+"&pg=0",
		success:function(m){
			var temp="";
		for(var i =0;i<m.length;i++){	
		var md = m[i];
		if(tp=='photo'){
		temp +='<a class="col-sm-3 col-xs-3 mo" id="mo'+md.id+'"onclick="PXMedia.selectMedia(\''+md.id+'\',\''+md.source+'\');" style="padding:5px">'+
				'<div class="img-responsive"><img src="'+md.thumb[75]+'"style="width:100%;height:80px"></div></a>';
		}else{
		temp +='<div class="gallery-item mo" id="mo'+md.id+'"><a onclick="PXMedia.selectMedia(\''+md.id+'\',\''+md.source+'\');">'+ 
                '<div class="btn vd_round-btn vd_bg-black-20 vd_white btn-lg mgtp-15"><i class="fa '+md.icon+'"></i></div>'+
                '<h3 class="filter-name mgtp-10 mgbt-xs-5 vd_grey font-semibold">'+md.title+'</h3>'+
                '<p class="vd_grey pdlr-10">'+md.filename+'</p>'+
                '<div class="bg-cover light"></div> </a></div>';
		}
		}$(".pxmob").html(temp);
		}
	});
	...
	this.next = function(){	
		this.pgn +=1;this.gopage(this.pgn);
	};
	...
	this.gopage=function(pg){
	this.pgn =pg;$(".pxmob").html('<img src="'+base_url+'/temp/img/loading.gif" />');var tpg =this.tp;
		$.ajax({
		url:base_url+"ajax.php",
		data:"class=Media_Core&function=getMediaAjax&tp="+this.tp+"&pg="+pg,
		success:function(m){
			var temp="";
		for(var i =0;i<m.length;i++){	
		var md = m[i];console.log(tpg);
		if(tpg == "photo"){
		temp +='<a class="col-sm-3 col-xs-3 mo" id="mo'+md.id+'"onclick="PXMedia.selectMedia(\''+md.id+'\',\''+md.source+'\');" style="padding:5px">'+
				'<div class="img-responsive"><img src="'+md.thumb[75]+'"style="width:100%;height:80px"></div></a>';
		}else{
	...
	this.selectMedia=function(n,m){		
	$(".mo").removeClass("vd_bg-twitter");
	$("#mo"+n).addClass("vd_bg-twitter");
	this.callback(n,m);
	};
	},
};

var PXMedia = new PX.Media();