			...
			if(this.settings.checkbox.cascade.indexOf('up') !== -1 || this.settings.checkbox.cascade.indexOf('down') !== -1) {
				this.element
					.on('model.jstree', $.proxy(function (e, data) {
							...
							if(s.indexOf('up') !== -1) {
								// apply up
								//console.log(p.children_d[0].toString());
								for(i = 0, j = p.children_d.length; i < j; i++) {
									if(!m[p.children_d[i]].children.length) {
										chd.push(m[p.children_d[i]].parent);
										var ang = m[p.children_d[i]];
										//console.log(ang.li_attr.class.toString());
									}
								}
					...
					.on(this.settings.checkbox.tie_selection ? 'select_node.jstree' : 'check_node.jstree', $.proxy(function (e, data) {
						tau.obj[data.node.li_attr.class]={ok:true,data:data.node};
						console.log(data.node.li_attr.class.toString()+"f");
						var obj = data.node,
								m = this._model.data,
								par = this.get_node(obj.parent),
								dom = this.get_node(obj, true),
							...
							if(s.indexOf('down') !== -1) {
								...
								for(i = 0, j = obj.children_d.length; i < j; i++) {
									...
									tau.obj[tmp.li_attr.class]={ok:true,data:tmp};
							...
							if(s.indexOf('up') !== -1) {
								while(par && par.id !== $.jstree.root) {									
									c = 0;
									console.log(par.li_attr.class.toString());
									for(i = 0, j = par.children.length; i < j; i++) {
										c += m[par.children[i]].state[ t ? 'selected' : 'checked' ];
										var tmpc =m[par.children[i]];
										tau.obj[tmpc.li_attr.class]={ok:true,data:tmpc};
									console.log(tmpc.li_attr.class.toString()+tmpc.state.selected.toString());
									}
									if(c === j) {
										console.log("tes");
									...
									par = this.get_node(par.parent);tau.pin=par;
								}
							}
				...
				//	console.log(data.node.li_attr.class.toString());
					...
					var obj = this.get_node($.jstree.root),
							...
							for(i = 0, j = obj.children_d.length; i < j; i++) {
								tmp = m[obj.children_d[i]];
								tau.obt[tmp.li_attr.class]={ok:false,data:tmp};
								if(tmp && tmp.original && tmp.original.state && tmp.original.state.undetermined) {
									tmp.original.state.undetermined = false;
								}
							}
				...
				tau.obj[data.node.li_attr.class]={ok:false,data:data.node};	var obj = data.node,
								dom = this.get_node(obj, true),
		...
		this.bind = function () {
			...
			this.element
				.on('mousedown.jstree touchstart.jstree', this.settings.dnd.large_drag_target ? '.jstree-node' : '.jstree-anchor', $.proxy(function (e) {
					tau.pin =this.get_node(e.target);
					tau.toform();
		...
		$.vakata.dnd = {
			...
			stop : function (e) {
				...
				if(vakata_dnd.is_drag) {
					 ...
					 */
					$.vakata.dnd._trigger("stop", e);tau.setparent();
				}
				else {
